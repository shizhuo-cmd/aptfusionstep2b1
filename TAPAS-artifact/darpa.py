...
def filters(filelist):
    ...
    padict = {}
    chdict = {}
    for file in tqdm(filelist, desc=f"Parsing", unit="file"):
        with open('./data/theia/logs/' + file, 'r', encoding='utf-8') as f:
            for line in f:
                js = json.loads(line)
    ...
    for subj in tqdm(subjhistory, desc=f"Getting node vector:", unit="node"):
        ...
        for eve in history:
            ...
            data.append(eve)
        if len(data) < 1:
            subjhisvec[subj] = [0.0] * 10
        else:
            train_x_tensor = torch.tensor(np.array([data]), dtype=torch.float32).to(device)
            output, h_n = LSTMmodel(train_x_tensor)
            vec = h_n[0]
            vec = torch.Tensor.tolist(vec)
            subjhisvec[subj] = vec
...
def dataenhance(x, addnum, onedataname):
    ...
    benignTop10actdict = {'cadets':[[5, 1, 2, 12, 0, 0], [10, 1, 2, 5, 0, 0], [5, 1, 2, 55, 10, 0], [5, 1, 2, 19, 0, 0], [5, 1, 2, 6, 8, 0],
               ...
               [5, 1, 2, 45, 10, 0], [5, 1, 2, 55, 10, 0], [8, 1, 3, 4, 2, 0], [5, 1, 2, 9, 0, 0], [5, 1, 2, 6, 8, 0],
               [6, 1, 3, 4, 2, 0], [8, 1, 3, 4, 1, 0], [6, 1, 3, 4, 1, 0], [5, 1, 2, 7, 0, 0], [10, 1, 2, 44, 0, 0]]}
...
def train(params, onedataname):
    ...
    data = torch.load('./data/{}/raw/data.pt'.format(onedataname))
...
def eval_final(data_name, model):
    ...
    dataset = torch.load('./data/cadets/raw/{}.pt'.format(data_name), weights_only=False)
