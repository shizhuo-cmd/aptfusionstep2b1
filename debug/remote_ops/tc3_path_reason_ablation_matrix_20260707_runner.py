...
OUTPUT_ROOT = REPO_ROOT / "debug" / "remote_ops" / "out" / "tc3_path_reason_ablation_matrix_20260707"
...
def _prepare_runtime_path_rules(dataset: str) -> tuple[Path, dict[str, Any]]:
    ...
    if dataset != "trace":
        return source_path, {"path_rules_profile": "default", "path_rules_source": str(source_path)}
    raw = yaml.safe_load(source_path.read_text(encoding="utf-8")) or {}
    if not isinstance(raw, dict):
        raise ValueError(f"Invalid path rules YAML: {source_path}")
    path_search = raw.setdefault("path_search", {})
    path_search["max_depth"] = 3
    path_search["top_k"] = 6
    path_search["pre_rank_limit"] = 12
    dataset_overrides = raw.setdefault("dataset_overrides", {})
    trace_override = dataset_overrides.setdefault("trace", {})
    trace_path_search = trace_override.setdefault("path_search", {})
    trace_path_search["max_total_span_minutes"] = min(
        int(trace_path_search.get("max_total_span_minutes", 120) or 120),
        60,
    )
    runtime_path = OUTPUT_ROOT / "trace_runtimecap_path_reason_rules_20260707.yaml"
    runtime_path.parent.mkdir(parents=True, exist_ok=True)
    runtime_path.write_text(yaml.safe_dump(raw, sort_keys=False, allow_unicode=True), encoding="utf-8")
    return runtime_path, {
        "path_rules_profile": "trace_runtime_cap_v1",
        "path_rules_source": str(source_path),
        "path_rules_runtime_path": str(runtime_path),
        "path_search_overrides": {
            "max_depth": 3,
            "top_k": 6,
            "pre_rank_limit": 12,
            "trace_max_total_span_minutes": 60,
        },
    }
...
def _variant_artifact_root(dataset: str, variant: str) -> Path:
    if dataset == "trace":
        return REPO_ROOT / (
            ...
            f"ablation_{variant}_e3gt_plus240_gtonly_20260707"
        )
    if dataset == "cadets":
        return REPO_ROOT / (
            f"artifacts_cadets_train_stats_latefusion_llama31_microstep2b_module1_gtbase_tactics_only_"
            f"ablation_{variant}_e3gt_plus240_20260707"
        )
...
def _dataset_spec(dataset: str) -> dict[str, Any]:
    if dataset == "trace":
        return {
            ...
            "reuse_from_source": ["module1", "module2"],
        }
    if dataset == "cadets":
        return {
            "dataset": dataset,
            ...
            "reuse_from_source": ["module3_evidence", "module4_compact"],
        }
    raise ValueError(f"Unsupported dataset: {dataset}")


def _baseline_variant_specs(dataset: str) -> list[dict[str, Any]]:
    baseline_reuse_mode = "source_module1" if dataset == "trace" else "source_module4"
...
def _run_variant(dataset: str, variant_spec: dict[str, Any], baseline_root: Path | None) -> dict[str, Any]:
    ...
    if dataset == "trace":
        cfg.path_top_k = min(int(getattr(cfg, "path_top_k", 20) or 20), 6)
