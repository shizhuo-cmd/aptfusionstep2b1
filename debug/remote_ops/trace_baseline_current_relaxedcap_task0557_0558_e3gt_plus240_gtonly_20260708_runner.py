...
ARTIFACT_ROOT = (
    ...
    "baseline_current_relaxedcap_task0557_0558_e3gt_plus240_gtonly_20260708"
)
GT_TIME_OFFSET_MINUTES = 240
TASK_IDS = {"task_0557", "task_0558"}
...
def _filter_module4_task_index(module4_dir: Path) -> None:
    task_index_path = module4_dir / "task_index.json"
    rows = json.loads(task_index_path.read_text(encoding="utf-8"))
    kept = [row for row in rows if str(row.get("task_id", "")).strip() in TASK_IDS]
    task_index_path.write_text(json.dumps(kept, ensure_ascii=False, indent=2), encoding="utf-8")

    compact_summary_path = module4_dir / "compact_summary.json"
    summary = json.loads(compact_summary_path.read_text(encoding="utf-8"))
    tasks = [row for row in summary.get("tasks", []) if str(row.get("task_id", "")).strip() in TASK_IDS]
    summary["task_count"] = len(tasks)
    summary["input_task_count"] = len(tasks)
    summary["kept_task_ids"] = [row["task_id"] for row in tasks]
    summary["filtered_out_task_ids"] = []
    summary["filtered_out_reasons"] = {}
    summary["tasks"] = tasks
    summary["kept_task_count"] = len(tasks)
    summary["filtered_out_task_count"] = 0
    compact_summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
...
def main() -> None:
    ...
    _filter_module4_task_index(cfg.artifacts_dir / "module4_compact")
    ...
    provenance = {
        "variant": "baseline_current_relaxedcap_task0557_0558",
