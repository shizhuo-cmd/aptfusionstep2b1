...
EXPECTED_GT_BASE_TASK_COUNT = 5
EXPECTED_MODULE1_TASK_COUNTS = {
    False: 5247,
    True: 5287,
}
REUSED_MODULE1_ROOTS = {
    False: REMOTE_REPO_ROOT
    / "artifacts_cadets_train_stats_latefusion_llama31_microstep2b_tactics_only_llm_gtonly_fanout_gt2_exclude_segmented_20260614"
    / "module1",
    True: REMOTE_REPO_ROOT
    / "artifacts_cadets_train_stats_latefusion_llama31_microstep2b_tactics_only_llm_gtonly_fanout_gt2_include_segmented_20260614"
    / "module1",
}
...
def _module1_task_rows(artifacts_root: Path) -> list[dict[str, Any]]:
    path = artifacts_root / "module1" / "task_subgraphs.json"
    rows = _load_json(path)
    return rows if isinstance(rows, list) else []


def _module1_summary(artifacts_root: Path) -> dict[str, Any]:
    return _load_json(artifacts_root / "module1" / "tapas_native_module1_summary.json")


def _module2_summary(artifacts_root: Path) -> dict[str, Any]:
    return _load_json(artifacts_root / "module2" / "task_subgraph_summary.json")


def _task_meta_rows(artifacts_root: Path) -> list[dict[str, Any]]:
    rows = _load_json(artifacts_root / "module2" / "task_meta_rich.json")
    return rows if isinstance(rows, list) else []
...
def _task_meta_by_id(artifacts_root: Path) -> dict[str, dict[str, Any]]:
    return {
        ...
        for row in _task_meta_rows(artifacts_root)
        if isinstance(row, dict) and str(row.get("task_id", "")).strip()
    }


def _task_index_rows(artifacts_root: Path) -> list[dict[str, Any]]:
    rows = _load_json(artifacts_root / "module3_evidence" / "task_index.json")
    return rows if isinstance(rows, list) else []


def _module3_selected_task_ids(artifacts_root: Path) -> list[str]:
    task_ids: list[str] = []
    for row in _task_index_rows(artifacts_root):
...
def _validation_summary(cfg: FusionConfig) -> dict[str, Any]:
    artifacts_root = cfg.artifacts_dir
    module1_rows = _module1_task_rows(artifacts_root)
    module1_summary = _module1_summary(artifacts_root)
    module2_summary = _module2_summary(artifacts_root)
    module2_meta_rows = _task_meta_rows(artifacts_root)
    module3_summary = _load_json(artifacts_root / "module3_evidence" / "summary.json")
    gt_positive_base_task_ids = _gt_positive_base_task_ids(artifacts_root)
    selected_task_ids = _module3_selected_task_ids(artifacts_root)
    expected_module1_task_count = EXPECTED_MODULE1_TASK_COUNTS[bool(cfg.task_component_count_segmented_children_upstream)]
    errors: list[str] = []

    module1_task_count = len(module1_rows)
    module2_task_count = len(module2_meta_rows)
    module2_summary_task_count = int(module2_summary.get("task_count", -1) or -1)
    module3_task_count = len(selected_task_ids)
    module3_summary_task_count = int(module3_summary.get("task_count", -1) or -1)

    if str(cfg.task_detector_mode).strip() != "load_and_predict":
        errors.append(f"task_detector_mode expected load_and_predict, got {cfg.task_detector_mode}")
    if module1_task_count != expected_module1_task_count:
        errors.append(
            f"module1 task_count expected {expected_module1_task_count}, got {module1_task_count}"
        )
    if int(module1_summary.get("task_count", -1) or -1) != expected_module1_task_count:
        errors.append(
            f"module1 summary task_count expected {expected_module1_task_count}, got {module1_summary.get('task_count')}"
        )
    if module2_task_count != module1_task_count:
        errors.append(f"module2 task_meta_rich count expected {module1_task_count}, got {module2_task_count}")
    if module2_summary_task_count != module1_task_count:
        errors.append(
            f"module2 summary task_count expected {module1_task_count}, got {module2_summary_task_count}"
        )
    if len(gt_positive_base_task_ids) != EXPECTED_GT_BASE_TASK_COUNT:
        errors.append(
            f"GT-positive base task count expected {EXPECTED_GT_BASE_TASK_COUNT}, got {len(gt_positive_base_task_ids)}"
        )
    if module3_task_count != EXPECTED_GT_BASE_TASK_COUNT:
        errors.append(f"module3 selected task_count expected {EXPECTED_GT_BASE_TASK_COUNT}, got {module3_task_count}")
    if module3_summary_task_count != EXPECTED_GT_BASE_TASK_COUNT:
        errors.append(
            f"module3 summary task_count expected {EXPECTED_GT_BASE_TASK_COUNT}, got {module3_summary_task_count}"
        )
    if selected_task_ids != gt_positive_base_task_ids:
        errors.append(
            "module3 selected task ids differ from GT-positive base task ids: "
            f"{selected_task_ids} != {gt_positive_base_task_ids}"
        )

    return {
        "passed": not errors,
        "errors": errors,
        "task_detector_mode": str(cfg.task_detector_mode),
        "task_detector_model_input": str(cfg.task_detector_model_input or ""),
        "module3_task_selection_mode": str(cfg.module3_task_selection_mode),
        "expected_module1_task_count": expected_module1_task_count,
        "expected_gt_positive_base_task_count": EXPECTED_GT_BASE_TASK_COUNT,
        "module1_task_count": module1_task_count,
        "module1_summary_task_count": int(module1_summary.get("task_count", -1) or -1),
        "module2_task_count": module2_task_count,
        "module2_summary_task_count": module2_summary_task_count,
        "module3_task_count": module3_task_count,
        "module3_summary_task_count": module3_summary_task_count,
        "gt_positive_base_task_ids": gt_positive_base_task_ids,
        "module3_selected_task_ids": selected_task_ids,
    }


def _module1_outputs_from_dir(module1_dir: Path) -> dict[str, Path]:
    return {
...
def _prepare_module1_outputs(cfg: FusionConfig) -> tuple[dict[str, Path], dict[str, Any]]:
    reused_root = REUSED_MODULE1_ROOTS[bool(cfg.task_component_count_segmented_children_upstream)]
    if reused_root.exists():
        _copy_tree(reused_root, cfg.module1_dir)
        return _module1_outputs_from_dir(cfg.module1_dir), {
            "module1_source_mode": "reused_prior_module1_artifacts",
            "module1_source_root": str(reused_root),
        }
    out1 = run_module1(cfg)
    return out1, {
        "module1_source_mode": "fresh_run_module1",
        "module1_source_root": "",
    }
...
def run_single_experiment(config_path: str | Path) -> dict[str, Any]:
    ...
    if str(cfg.task_detector_mode).strip() != "load_and_predict":
        raise ValueError(f"CADETS all-task GT-direct run requires task_detector_mode=load_and_predict, got {cfg.task_detector_mode}")
    if cfg.task_detector_model_input is None or not Path(cfg.task_detector_model_input).exists():
        raise FileNotFoundError(
            f"task_detector_model_input is required for load_and_predict and must exist: {cfg.task_detector_model_input}"
        )
    _clean_dir(cfg.artifacts_dir)
    cfg.artifacts_dir.mkdir(parents=True, exist_ok=True)

    out1, module1_provenance = _prepare_module1_outputs(cfg)
    out2 = run_module2(
        cfg=cfg,
    ...
    validation = _validation_summary(cfg)
    ...
    provenance = {
        ...
        "experiment_semantics": "module1_all_tasks -> module2_load_and_predict_all_tasks -> module3_ground_truth_positive_base_only",
        "task_detector_mode": str(cfg.task_detector_mode),
        "task_detector_model_input": str(cfg.task_detector_model_input or ""),
    ...
    validation_path = cfg.artifacts_dir / "validation_summary.json"
    ...
    if not validation["passed"]:
        raise RuntimeError("CADETS GT-direct run validation failed: " + " | ".join(validation["errors"]))
    return summary
