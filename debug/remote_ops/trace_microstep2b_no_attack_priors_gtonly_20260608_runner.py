...
def _evaluate(cfg: FusionConfig) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    output_dir = cfg.artifacts_dir / "path_reason_eval_no_attack_priors"
...
def _run_compare(full_prior_root: Path, no_prior_root: Path) -> dict[str, str]:
    output_dir = no_prior_root / "prior_effect_analysis"
    command = [
        sys.executable,
        str(COMPARE_SCRIPT),
...
def main() -> None:
    ...
    compare_outputs = _run_compare(full_prior_root, cfg.artifacts_dir)
    ...
    provenance = {
        ...
        "full_prior_experiment_root": str(full_prior_root),
    ...
    decision_summary = {
        ...
        "full_prior_experiment_root": str(full_prior_root),
        "full_prior_metrics_path": str(full_prior_metrics_path),
        "full_prior_metrics": _load_metrics(full_prior_metrics_path),
        "no_prior_artifacts_root": str(cfg.artifacts_dir),
        "no_prior_metrics": metrics,
