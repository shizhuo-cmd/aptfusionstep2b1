...
CONFIG_PATH = Path(
    "configs/fusion_cloud_cadets_train_stats_latefusion_llama31_microstep2b_alltasks_loadpredict_gtbase_tactics_only_llm_fanout_gt2_exclude_segmented_step1_chainfix_20260617.yaml"
)
...
def _extra_tactic_failures(current_rows: list[dict[str, Any]], baseline_rows: list[dict[str, Any]]) -> list[str]:
    ...
    for window_id in sorted(TARGET_WINDOW_IDS):
        ...
        if current_extra > baseline_extra:
            failures.append(
                f"{window_id} extra_tactics increased from {baseline_extra} to {current_extra}"
            )
    return failures
