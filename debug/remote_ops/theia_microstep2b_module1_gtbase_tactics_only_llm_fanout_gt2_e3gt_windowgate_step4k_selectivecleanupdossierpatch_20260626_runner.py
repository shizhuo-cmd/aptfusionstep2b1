...
TARGET_ROOT = (
    REPO_ROOT
    / "artifacts_theia_train_stats_latefusion_llama31_microstep2b_module1_gtbase_tactics_only_llm_fanout_gt2_e3gt_windowgate_step4k_selectivecleanupdossierpatch_offset240_20260626"
)
...
def _selective_cleanup_refresh(cfg) -> dict[str, Any]:
    ...
    for source_file in sorted(source_dir.glob("*.json")):
        ...
        for payload in raw_items if isinstance(raw_items, list) else []:
            ...
            if "cleanup_delete" not in set(path.family_tags) or cleanup_summary:
            ...
            path.followup_event_ids = _sorted_unique(extra_cleanup_ids + list(path.followup_event_ids))[:8]
            path.support_event_ids = _sorted_unique(list(path.support_event_ids) + list(extra_cleanup_ids))
            ...
            summary["patched_paths"].append(
                {
                    ...
                    "added_cleanup_event_ids": list(extra_cleanup_ids),
...
def main() -> None:
    ...
    patch_summary = _selective_cleanup_refresh(cfg)
    module6_outputs = run_module6_reason(cfg)
    metrics, eval_outputs = _evaluate(cfg)

    provenance = {
        "experiment_step": "step4k_selectivecleanupdossierpatch",
