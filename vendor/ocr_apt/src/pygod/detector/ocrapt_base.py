...
class Detector(ABC):
    ...
    @abstractmethod
    def fit(self, data, label=None,fig_title=None):
    ...
    def predict(self,
        ...
        if data is None:
            ...
            score = self.decision_function(data, label,batch_size)
        if return_pred:
    ...
    def _process_decision_score(self):
        ...
        """
        print("Final contamination is:", self.contamination)
...
class OCRAPT_DeepDetector(Detector, ABC):
    """
    ...
    **kwargs
    ...
    def __init__(self,
        ...
        super(OCRAPT_DeepDetector, self).__init__(contamination=contamination,
                                           verbose=verbose)
        ...
        while retrain:
            ...
            for epoch in range(self.epoch):
                ...
                for sampled_data in loader:
                    ...
                    if (epoch % 10 == 0) or (epoch == self.epoch-1):
                        loss, score = self.forward_model(sampled_data, visualize=self.visualize, fig_title=str(fig_title+"epoch_"+str(epoch)+".png"))
                    else:
    ...
    def decision_function(self, data, label=None,batch_size=None):
        ...
        for sampled_data in loader:
            if self.visualize:
                loss, score = self.forward_model(sampled_data, train=False,visualize=self.visualize,label=label,fig_title=self.fig_title)
            else:
                loss, score = self.forward_model(sampled_data,train=False)
            batch_size = sampled_data.batch_size
            node_idx = sampled_data.n_id
            if self.save_emb:
                if type(self.hid_dim) is tuple:
    ...
    def predict(self,
        ...
        output = super(OCRAPT_DeepDetector, self).predict(data,
                                                   label,
                                                   batch_size,
                                                   fig_title,
                                                   return_pred,
    ...
    def validate_model(self, data, visualize=False,fig_title=None):
        """
        get validation loss for the one class classifier detector.
        ...
        -------
        early_stop : bool
                    0 to continue training, 1 to stop training, 2 to stop training and retrain with more contamination rate,
                    3 to stop training and retrain with less contamination rate.
        """
